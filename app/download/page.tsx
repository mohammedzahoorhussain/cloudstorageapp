...
export default function DownloadPage() {
  ...
  // verify user by checking file_access row(s) for user_name and comparing bcrypt hash
  const verifyUser = async () => {
    ...
    // Compare password with the first row's hash (all rows for same user should share same password)
  ...
  // fetch files from storage then filter by username_ prefix
  ...
  const deleteFile = async (fileName: string) => {
    ...
    // delete DB entries for that file
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-10 px-4">
      <header className="absolute top-0 left-0 right-0 flex justify-between items-center p-4 bg-white shadow-md">
        ...
        <nav className="space-x-4">
          ...
          <Link href="/upload" className="text-gray-800 hover:text-blue-600 font-medium">Upload</Link>
          <Link href="/download" className="text-blue-600 font-semibold">Download</Link>
        </nav>
      </header>
      ...
      {!verified ? (
        <div className="bg-white rounded-2xl shadow-lg p-8 mt-20 w-full max-w-md text-center">
          ...
          <p className="text-gray-800 mb-6 font-medium">Enter your name and password to view your uploaded files.</p>
        ...
        <div className="mt-24 w-full max-w-3xl bg-white rounded-2xl shadow-lg p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-blue-700">📂 Files for {userName}</h2>
            <button onClick={handleLogout} className="text-sm text-gray-700 hover:text-blue-600 font-medium">Log Out</button>
          </div>

          {files.length === 0 ? (
            <p className="text-center text-gray-700 font-medium">No files found for this user.</p>
          ) : (
            <ul className="space-y-3">
              {files.map((file) => (
                <li key={file.name} className="flex justify-between items-center bg-gray-100 p-3 rounded-lg shadow-sm">
                  <span className="truncate max-w-[60%] text-gray-900 font-semibold">
                    {file.name.replace(userName + "_", "")}
                  </span>

                  <div className="flex gap-3">
                    <a href={getFileUrl(file.name)} target="_blank" rel="noopener noreferrer" className="text-blue-700 font-bold hover:underline">Download ⬇️</a>
                    <button onClick={() => deleteFile(file.name)} disabled={deleting === file.name} className="text-red-600 font-bold hover:underline">
